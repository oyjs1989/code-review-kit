...
**在 goroutine 或异步处理中发现问题后，必须读完整个外层函数**，验证调用方是否有兜底机制，再决定严重程度。

**检查点**：
- `errGroup.Wait()` / `wg.Wait()` 之后，调用方是否有重试逻辑？
- 是否有超时检查（如 `time.Since(startTime) > maxDuration`）？
- 失败是否会进 DLQ / 死信队列？
- Kafka 消费者：消费失败时消息是否会被重新消费（重试）？

**判断原则**：
- 有兜底机制 → 降级（P0→P2），问题性质变为"异常难以排查"而非"永久卡死/数据丢失"
- 无任何兜底 → 维持高优先级
