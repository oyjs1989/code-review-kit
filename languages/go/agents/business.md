...
异步处理（消息队列、定时任务、事件驱动）中，单次处理失败不等于业务失败——必须追踪**完整兜底链路**再判断严重程度。

**检查要点**：
- 处理失败后，任务是否会被重新触发（队列重投递、定时轮询、调度重试）？
- 是否有超时检测，超时后是否强制终态（避免永久 Processing）？
- 重试是否有上限，上限后是否有死信/人工介入机制？
- 重试与业务层补偿是否可能叠加产生重复操作（如重复扣款）？
